...
function ProjectItem({
  ...
  imageUrl,
  borderColor,
  largeContent = false, // New prop to handle the border-radius for larger items
}: {
  name: string;
  url: string;
  imageUrl: string;
  borderColor: string;
  largeContent?: boolean; // Optional prop for larger content
}) {
  return (
    <div
      className={`p-4 border-2 ${
        largeContent ? "rounded-lg" : "rounded-full"
      } shadow-lg transform transition-transform duration-200 hover:scale-105 ${borderColor} bg-[#00aff0]`}
    >
      <a
        href={url}
...
export default function ProfileProjects() {
  return (
    <div className="bg-white p-4 space-y-16">
      <div className="flex flex-col space-y-8"> {/* Removed md:flex-row and md:space-x-8 */}
